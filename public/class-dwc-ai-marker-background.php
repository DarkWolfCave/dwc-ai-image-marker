...
class Dwc_Ai_Marker_Background {
	...
	/**
	 * Bereitet Bild-Daten für die JavaScript-Verarbeitung vor.
	 *
	...
	private function prepare_image_data() {
		...
		if ( false === $count ) {
			global $wpdb;
			// Für Zählungen ist eine direkte SQL-Abfrage effizienter als meta_query.
			...
			// phpcs:enable
			wp_cache_set( $count_cache_key, $count, 'dwc_ai_marker', HOUR_IN_SECONDS );
		}
		...
		// 2. Abrufen der KI-Bild-IDs mit Objekt-Cache.
		$ai_image_ids = wp_cache_get( $ids_cache_key, 'dwc_ai_marker' );

		if ( false === $ai_image_ids ) {
			// Zuerst versuchen, aus Transient zu laden (längerfristiger Cache).
			$ai_image_ids = get_transient( $ids_cache_key );

			if ( false === $ai_image_ids ) {
				// Optimierter Ansatz: Direkten SQL-Query für IDs verwenden.
				global $wpdb;
				// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery
				...
				// Cache in Transient für längere Zeit (24 Stunden).
				set_transient( $ids_cache_key, $ai_image_ids, DAY_IN_SECONDS );
			}

			// Cache im Objekt-Cache für schnellen Zugriff während der Anfrage.
			wp_cache_set( $ids_cache_key, $ai_image_ids, 'dwc_ai_marker', HOUR_IN_SECONDS );
		}
		...
		// 3. KI-Bild-Attachments mit Objekt-Cache.
		$ai_image_attachments = wp_cache_get( $attachments_cache_key, 'dwc_ai_marker' );

		if ( false === $ai_image_attachments ) {
			$ai_image_attachments = array();

			// Hier verwenden wir get_posts effizient, indem wir:
			// 1. Nach den spezifischen IDs filtern (kein meta_query nötig).
			// 2. Nur die ersten X Bilder laden (um große Abfragen zu vermeiden).
			$limit = min( count( $ai_image_ids ), 100 ); // Maximal 100 Bilder verarbeiten.
			$args  = array(
				'post_type'      => 'attachment',
				'post_status'    => 'inherit',
				'posts_per_page' => $limit,
				'post__in'       => array_slice( $ai_image_ids, 0, $limit ),
			...
			wp_cache_set( $attachments_cache_key, $ai_image_attachments, 'dwc_ai_marker', HOUR_IN_SECONDS );
		}

		return array(
			'ai_image_ids'         => $ai_image_ids,
			'ai_image_attachments' => $ai_image_attachments,
		);
	}
	...
	public function add_dynamic_background_handler() {
		// Hole vorbereitete Bilddaten.
		$image_data = $this->prepare_image_data();

		// Wenn keine Daten vorhanden, beende frühzeitig.
		if ( empty( $image_data ) ) {
			return;
		}

		$ai_image_ids         = $image_data['ai_image_ids'];
		$ai_image_attachments = $image_data['ai_image_attachments'];
				...
				function processImages() {
					...
					const checkImageId = (url) => {
						...
						if (idMatch && idMatch[1]) {
							const id = idMatch[1].substring(1); // Entferne das erste Zeichen (- oder _)
							if (aiImageLookup[id]) {
								debug(`KI-Bild durch ID erkannt: ${id}`);
								return true;
							}
						}
