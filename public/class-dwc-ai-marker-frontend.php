...
class Dwc_Ai_Marker_Frontend {
	...
	public function init_hooks() {
		...
		add_filter( 'render_block_generateblocks/container', array( $this, 'process_generateblocks_container_background' ), 10, 2 );
	...
	public function process_content_images( $content ) {
		...
		if ( strpos( $content, '--background-url:url(' ) !== false ) {
			...
			if ( isset( $bg_matches[1] ) && !empty( $bg_matches[1] ) ) {
				foreach ( $bg_matches[1] as $key => $bg_url ) {
					...
					if ( $image_id && dwc_ai_marker()->utils->is_ai_generated( $image_id ) ) {
						...
						if ( isset( $container_match[0] ) && strpos( $container_match[0], 'ai-image-badge' ) === false ) {
							$options = dwc_ai_marker()->get_options();
		...
		if ( ! empty( $matches[0] ) ) {
			...
			for ( $i = 0; $i < $max_images; $i++ ) {
				...
				if ( strpos( $img_tag, 'ai-image-badge' ) !== false ) {
					continue;
				}
				...
				if ( $image_id > 0 && dwc_ai_marker()->utils->is_ai_generated( $image_id ) ) {
					...
					if ( strpos( $content, '<div class="ai-image-wrapper">' . $img_tag . '</div>' ) === false &&
						strpos( $content, '<div class="ai-image-wrapper">' . PHP_EOL . $img_tag ) === false ) {
	...
	public function process_generate_blocks_image( $block_content, $block ) {
		...
		if ( strpos( $block_content, 'ai-image-badge' ) !== false ) {
			return $block_content;
		}
		...
		if ( $image_id && dwc_ai_marker()->utils->is_ai_generated( $image_id ) ) {
			...
			if ( strpos( $block_content, '<img' ) !== false && strpos( $block_content, '</img>' ) === false ) {
	...
	public function process_generateblocks_container_background( $block_content, $block ) {
		...
		if ( isset( $block['attrs']['backgroundImage'] ) && ! empty( $block['attrs']['backgroundImage'] ) ) {
			...
			} else if ( isset( $background_image['url'] ) && ! empty( $background_image['url'] ) ) {
		...
		if ( strpos( $block_content, '--background-url:url(' ) !== false ) {
	...
	public function process_attachment_image( $html, $attachment_id, $size, $icon, $attr ) {
		...
		if ( strpos( $html, 'ai-image-badge' ) !== false ) {
			return $html;
		}
	...
	public function process_generatepress_blog_image( $image_html, $post_id ) {
		// Schnelle Prüfung, ob bereits markiert
		if ( strpos( $image_html, 'ai-image-badge' ) !== false ) {
			return $image_html;
		}
